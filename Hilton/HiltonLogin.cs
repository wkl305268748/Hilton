...
namespace Hilton
{
    public class HiltonLogin
    {
        ...
        private List<CookieItem> getWebCookieTest()
        {
            string cookie = GetCookieString("https://secure3.hilton.com/en/hh/customer/login/index.htm");
            if (cookie == null)
                return new List<CookieItem>();

            List<CookieItem> cookieList = HttpCookieHelper.GetCookieList(cookie);
        ...
        List<CookieItem> cookieItem = new List<CookieItem>();
        ...
        private List<CookieItem> getSensorTest(string cookie)
        {
            string postdata = hiltonBM.getBm();
        ...
        private void subCookie(List<CookieItem> from) {
            foreach (CookieItem from_item in from) {
                ...
                foreach (CookieItem final_item in cookie_final)
                {
                    ...
                    {
                        find = true;
                        //final_item.Value = from_item.Value;
                    }
                }
                if (!find)
                {
                    //Console.WriteLine(http.Key + "---" + http.Value);
        ...
        string key_bm = "";
        public int setNextKey() {
            return hiltonBM.getNext();
        }
        ...
        public string getPointsTest(string username, string password)
        {
            //List<CookieItem> cookie_web = getWebCookieTest();
            //subCookie(cookie_web);
            ...
            List<CookieItem> cookie_sensor = getSensorTest(getString(cookie_final));
            if (cookie_sensor == null)
                return "timeout";
            subCookie(cookie_sensor);

            //Console.WriteLine(getString(cookie_final));
            ...
            {
                //Console.WriteLine(result.Header.ToString());
                //
